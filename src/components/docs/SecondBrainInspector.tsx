...
export default function SecondBrainInspector({
  ...
  const handleSaveNotionUrl = () => {
    ...
    if (!isValidEmbedUrl(trimmed)) {
      alert('URL no permitida por políticas de seguridad (debe ser HTTPS de un dominio autorizado de Notion).');
      return;
    }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ background: '#000000', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '18px', overflow: 'hidden', height: '500px', display: 'flex', flexDirection: 'column' }}>
              ...
              {isValidEmbedUrl(notionEmbedUrl) ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--color-accent-danger)', padding: '20px', textAlign: 'center' }}>
                  ⚠️ URL de Notion no válida o no permitida por políticas de seguridad.
                </div>
              )}
            </div>
          </div>
