...
export function YouTubePlayer({
  ...
  const renderPlayerContent = () => {
    ...
    if (isVimeo) {
      ...
      return isValidEmbedUrl(vimeoEmbedUrl, ALLOWED_VIDEO_DOMAINS) ? (
        <iframe
          key={vimeoEmbedUrl}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-tertiary, rgba(255,255,255,0.5))', fontSize: '0.82rem' }}>
          Video no disponible o URL no autorizada
        </div>
      );
    }
    ...
    if (isYouTube) {
      ...
      return isValidEmbedUrl(embedUrl, ALLOWED_VIDEO_DOMAINS) ? (
        <iframe
          key={embedUrl}
          style={{ width: '100%', height: '100%', border: 'none' }}
          src={embedUrl}
