  ...
  const renderPlayerContent = () => {
    ...
    if (isVimeo) {
      ...
      if (!isValidEmbedUrl(vimeoEmbedUrl)) {
        return (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--color-accent-danger, #ff453a)', fontSize: '0.84rem' }}>
            URL de video Vimeo no válida o no permitida.
          </div>
        );
      }
    ...
    if (isYouTube) {
      ...
      if (!isValidEmbedUrl(embedUrl)) {
        return (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--color-accent-danger, #ff453a)', fontSize: '0.84rem' }}>
            URL de video YouTube no válida o no permitida.
          </div>
        );
      }
