  ...
  const renderPlayerContent = () => {
    ...
    if (isVimeo) {
      ...
      return isValidEmbedUrl(vimeoEmbedUrl) ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-tertiary)', padding: '16px' }}>
          ⚠️ Enlace de Vimeo no seguro o no permitido.
        </div>
      );
    }
    ...
    if (isYouTube) {
      ...
      return isValidEmbedUrl(embedUrl) ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-tertiary)', padding: '16px' }}>
          ⚠️ Enlace de YouTube no seguro o no permitido.
        </div>
      );
    }
