  ...
  const renderPlayerContent = () => {
    ...
    if (isVimeo) {
      ...
      if (!isValidEmbedUrl(vimeoEmbedUrl)) {
        return (
          <div style={{ padding: '16px', color: 'var(--color-accent-danger, #ff453a)', textAlign: 'center' }}>
            URL de embed de Vimeo no permitida por políticas de seguridad.
          </div>
        );
      }
    ...
    if (isYouTube) {
      ...
      if (!isValidEmbedUrl(embedUrl)) {
        return (
          <div style={{ padding: '16px', color: 'var(--color-accent-danger, #ff453a)', textAlign: 'center' }}>
            URL de embed de YouTube no permitida por políticas de seguridad.
          </div>
        );
      }
