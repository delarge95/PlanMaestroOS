    ...
    <ErrorBoundary>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
          <div>
            <span style={{ fontSize: '0.72rem', fontFamily: 'SF Mono, monospace', color: 'var(--color-state-done)', fontWeight: 700, textTransform: 'uppercase' }}>
          ...
          <div style={{ display: 'flex', gap: '4px', background: 'rgba(255,255,255,0.06)', padding: '4px', borderRadius: '12px' }}>
            <button
              ...
              style={{ background: selectedRange === 'week' ? 'var(--color-state-done)' : 'transparent', color: selectedRange === 'week' ? '#ffffff' : 'var(--color-text-secondary)', border: 'none', padding: '6px 12px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
            >
              Semana
            </button>
            <button
              ...
              style={{ background: selectedRange === 'month' ? 'var(--color-state-done)' : 'transparent', color: selectedRange === 'month' ? '#ffffff' : 'var(--color-text-secondary)', border: 'none', padding: '6px 12px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
            >
              Mes
            </button>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--color-border-subtle)', borderRadius: '16px', padding: '16px' }}>
            <span style={{ fontSize: '0.72rem', color: 'var(--color-state-done)', fontWeight: 700, textTransform: 'uppercase' }}>ADHERENCIA AM / PM</span>
            <strong style={{ fontSize: '1.8rem', display: 'block', margin: '4px 0', color: 'var(--color-text-primary)' }}>94%</strong>
          ...
          <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--color-border-subtle)', borderRadius: '16px', padding: '16px' }}>
            ...
            <strong style={{ fontSize: '1.8rem', display: 'block', margin: '4px 0', color: 'var(--color-text-primary)' }}>101%</strong>
          ...
          <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--color-border-subtle)', borderRadius: '16px', padding: '16px' }}>
            ...
            <strong style={{ fontSize: '1.8rem', display: 'block', margin: '4px 0', color: 'var(--color-text-primary)' }}>1.0 / 10</strong>
        ...
        <div style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid var(--color-border-subtle)', borderRadius: '18px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '20px', height: '180px', padding: '10px 0', borderBottom: '1px solid var(--color-border-subtle)' }}>
            {VOLUME_DATA.map((d, idx) => {
              ...
              return (
                <div key={idx} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end', gap: '8px' }}>
                  ...
                  <div style={{ width: '36px', height: `${heightPct}%`, background: 'linear-gradient(180deg, var(--color-state-done), var(--color-state-done-soft))', borderRadius: '8px' }} />
        ...
        <div style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid var(--color-border-subtle)', borderRadius: '18px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <h4 style={{ fontSize: '1rem', margin: 0, color: 'var(--color-text-primary)', fontWeight: 700 }}>
            ...
            {SKILL_PROGRESSION.map((sk, idx) => (
              <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem' }}>
                  <strong style={{ color: 'var(--color-text-primary)' }}>{sk.skill}</strong>
                  <span style={{ color: sk.color, fontWeight: 700 }}>{sk.level} ({sk.progress}%)</span>
                </div>
                <div style={{ background: 'var(--color-border-subtle)', height: '6px', borderRadius: '999px', overflow: 'hidden' }}>
